...
<template>
  <div ref="container" class="border border-gray-300 rounded relative flex justify-start items-center p-3 mt-3 bg-white font-sans cursor-text my-auto mb-0 transition-all" @click="input.focus()" :class="props.hasRule ? `${props.class}` : `${props.class}`">
    <input ref="input" type="text" :id="props.placeholder" class="w-full outline-none bg-transparent" @update:model-value="emits('update:modelValue', text)" v-model="text" @blur="validate">
